...
int main(int argc, char** argv)
{
  ...
  else if (argc == 4) { inf = atoi(argv[1]); sup = atoi(argv[2]); inc=atoi(argv[3]);}
  else { printf("Usage: ./exe TAM test\n"); exit(0); }

  // numero de Threads en cada dimension 
  nThreads = SIZE;
  for ( N = inf; N <= sup; N+=inc) {
	  // numero de Blocks en cada dimension 
	  nBlocks = ceil(double(N+nThreads-1)/nThreads);
	  ...
	  // Ejecutar el kernel 
	  Kernel02<<<dimGrid, dimBlock>>>(N, N, N, d_A, d_B, d_C);
	  ...
	  int pass = TestMM(N,N,N,h_A,h_B,h_C);
	  printf("%d-PASS:%d: %f %f\n", N, pass, \
